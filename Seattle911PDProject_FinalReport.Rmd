...
This is the `r rmarkdown::metadata$title` for `r rmarkdown::metadata$info_course` from `r rmarkdown::metadata$info_program` offered by `r rmarkdown::metadata$info_institution`. Report is prepared by `r rmarkdown::metadata$author`.
...
#### Was there increase/decrease in event clearance description overall occurrence over time?

We are building Timeline plot to see this.
...
Here we have locations that have persistent occurences of ECDs by average and median. Now, let us see whether same ECDs occur at these locations. We count the groups of ECDs that have occurence for each of those locations and average the results by ECD.

Let us see where exactly these points lie on the map by using Latitude and Longitude. And how intense they look. But we are not using every location. Just the top 1000 locations by ocurrence of ECDs

```{r,plot-ecd-intensity-map,fig.cap='Seattle 911 Incident Response: ECD Intensity Map', fig.subcap='', out.width="80%", echo=FALSE}
knitr::include_graphics("images/Rplot14_ECDIntensityMap_OverallSeattle.png")
```
...
```{r,plot-ecd-intensity-map-month,fig.cap='Seattle 911 Incident Response: ECD Intensity Map Faceted by Month', fig.subcap='', out.width="80%", echo=FALSE}
knitr::include_graphics("images/Rplot15_ECDIntensityMap_FacetedByMonth_Top100.png")
```

We can do the same for weekdays. Here we are considering daily totals averaged by Weekdays.

```{r,plot-ecd-intensity-map-weekday,fig.cap='Seattle 911 Incident Response: ECD Intensity Map Faceted by Month', fig.subcap='', out.width="80%", echo=FALSE}
knitr::include_graphics("images/Rplot16_ECDIntensityMap_FacetedByWeekday_Top100.png")
```
...
```{r pvalue-ecd-iloc-pair, eval=FALSE}
...
```{r kmeans-cluster-summary, eval=FALSE}
#ECD~ILoc pair results
...
```{r kmeans-cluster-group-size, eval=FALSE}
...
|2         |      2129||61        |      1215||79        |       783|
|...|

```{r,kmeans-cluster-map-by-location,fig.cap='Seattle 911 Incident Response: KMeans Clusters Map By Location', fig.subcap=c('Larges Group','Other 99 Groups'), fig.show="hold", out.width="50%", echo=FALSE}
knitr::include_graphics(c("images/Rplot17_KmeansClustersMapByLocation_100C_14.png","images/Rplot17_KmeansClustersMapByLocation_100C.png"))
```

Further, we will train the **S911IR** data with knn() algorithm.

\newpage

## Final model performance

With analysis, we have determined that ECD is largely dependent on location, than any other feature.
Therefore we are proceeding with tuning a knn() model.

We have used this script for tuning *k* value for knn3() algorithm

```{r knn-training-tuning, eval=FALSE}
...
We have tuned the knn with two separate sequences (9:200 and 200:300) to arrive at the final *k* value.
And we see the plot for this tuning process as follows:

```{r,plot-knn-tuning, fig.cap='k Parameter Tuning', fig.subcap=c('k = seq(9, 200, 2)','k = seq(200, 300, 2)'), fig.show="hold", out.width="50%", echo=FALSE}
knitr::include_graphics(c("images/Rplot_KnnTuning.png","images/Rplot_KnnTuning_k292.png"))
```

We fit the model with following code:

And the confusion matrix of the model trained with knn() algorithm is as follows:
...
```{r,plot-knn-tuning, fig.cap='Random Forest Parameter Tuning', fig.subcap=c('ntree = 100','ntree = 500'), fig.show="hold", out.width="50%", echo=FALSE}
knitr::include_graphics(c("images/Rplot_RF_MTry_Ntree100_nSamp10000_LatLongITDescNECDay.png","images/Rplot_RF_MTry_Ntree500_nSamp10000_LatLongITDescNECDay.png"))
```
...
We have trained basic and not-so-precise machine-learning algorithm. Accuracy, Sensitivity and Specifity results should be improved further for better use in real life.
...
## Tests and evaluation to be done
...
## Technical requirements for the execution

Mobile technology, government regulations restrict Mongolia from tracking the phone calls to its source. Prior to analyzing and training the data, we would need to improve the tracking ability for 102 and 103 operators.
